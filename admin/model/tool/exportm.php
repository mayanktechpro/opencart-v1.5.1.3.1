...
class ModelToolExportm extends Model {
	...
	function storeManufacturersIntoDatabase( &$database, &$products, &$manufacturerIds ) {
		...
		$result = $database->query( $sql );
		if ($result->rows) {
			$saved_manufacturers = array();
			foreach ($result->rows as $row) {
				$manufacturerId = $row['manufacturer_id'];
				$name = $row['name'];
				$saved_manufacturers[$manufacturerId] = $name;
		...
		foreach ($products as $product) {
			...
			if ($manufacturerName=="") {
				continue;
			} else if(in_array($manufacturerName, $saved_manufacturers)) {
				continue;
			}
	...
	function storeRomanianProductsIntoDatabase( &$database, &$products ) 
	{
		// find the default language id
		$languageId = 2;
		
		// start transaction, remove previous defined descriptions in Romanian for products
		$sql = "START TRANSACTION;\n";
		$sql .= "DELETE FROM `".DB_PREFIX."product_description` WHERE language_id=$languageId;\n";

		$this->import( $database, $sql );
		
		// generate and execute SQL for storing the descriptions for products
		foreach ($products as $product) {
			$productId = $product['product_id'];
			$productName = $database->escape($product['name']);
			$productDescription = $database->escape($product['description']);
	...
	function uploadRomanianProducts( &$reader, &$database ) {
		// find the default language id and default units
		$languageId = 2;
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			$name = $this->getCell($data,$i,$j++);
			$name = htmlentities( $name, ENT_QUOTES, $this->detect_encoding($name) );
			$langId = $this->getCell($data,$i,$j++,'2');
			if ($langId!=$languageId) {
				continue;
			}
			$description = $this->getCell($data,$i,$j++);
		...
		return $this->storeRomanianProductsIntoDatabase( $database, $products );
	}
	...
	function existOptionValueImageField( &$database ) {
		$hasOptionValueImageField = FALSE;
		$result = $this->db->query( "DESCRIBE `".DB_PREFIX."option_value`;" );
		foreach ($result->rows as $row) {
			if ($row['Field'] == 'image') {
				$hasOptionValueImageField = TRUE;
				break;
			}
		}
		return $hasOptionValueImageField;
	}
	...
	function storeOptionsIntoDatabase( &$database, &$options ) 
	{
		...
		// check for the existence of the option_value.image field which was only introduce with OpenCart version 1.5.1.3
		$hasOptionValueImageField = $this->existOptionValueImageField( $database );
		...
		$po_count =  $this->db->query("SELECT COUNT(*) AS count FROM `".DB_PREFIX."product_option")->row;
		$pov_count =  $this->db->query("SELECT COUNT(*) AS count FROM `".DB_PREFIX."product_option_value")->row;
		$maxProductOptionId = $po_count['count'];
		$maxProductOptionValueId = $pov_count['count'];
		$countOptions = 0;
		...
		foreach ($options as $option) {
			...
			if (($type=='select') || ($type=='checkbox') || ($type=='radio') || (($hasOptionValueImageField) && ($type=='image'))) {
	...
	function storeSpecialsIntoDatabase( &$database, &$specials )
	{
		...
		//$productSpecialId = 0;
		$special_count = $this->db->query("SELECT COUNT(*) AS count FROM `".DB_PREFIX."product_special")->row;
		$productSpecialId = $special_count['count'];
		$first = TRUE;
	...
	function storeDiscountsIntoDatabase( &$database, &$discounts )
	{
		...
		//$productDiscountId = 0;
		$discount_count = $this->db->query("SELECT COUNT(*) AS count FROM `".DB_PREFIX."product_discount")->row;
		$productDiscountId = $discount_count['count'];
		$first = TRUE;
	...
	function storeRewardsIntoDatabase( &$database, &$rewards )
	{
		...
		//$productRewardId = 0;
		$reward_count = $this->db->query("SELECT COUNT(*) AS count FROM `".DB_PREFIX."product_reward")->row;
		$productRewardId = $reward_count['count'];
		$first = TRUE;
	...
	function storeAdditionalImagesIntoDatabase( &$reader, &$database )
	{
		...
		//$maxImageId = 0;
		$image_count = $this->db->query("SELECT COUNT(*) AS count FROM `".DB_PREFIX."product_image")->row;
		$maxImageId = $image_count['count'];
	...
	function validateRomanianProducts( &$reader )
	{
		$expectedRomanianHeading = array("product_id", "name", "language_id", "description", "meta_description", "meta_keywords" );
		$data =& $reader->getSheet(0);
		return $this->validateHeading( $data, $expectedRomanianHeading );
	}
	...
	function validateUploadRomanian( &$reader ) {
		if ($reader->getSheetCount() != 1) {
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get( 'error_sheet_count_romanian' )."\n",3,DIR_LOGS."error.txt");
			return FALSE;
		}
		if (!$this->validateRomanianProducts( $reader )) {
			error_log(date('Y-m-d H:i:s - ', time()).$this->language->get('error_romanian_header')."\n",3,DIR_LOGS."error.txt");
			return FALSE;
		}
	}
	...
	function uploadRomanian( $filename ) {
		global $config;
		global $log;
		$config = $this->config;
		$log = $this->log;
		...
		$objReader->setReadDataOnly(true);
		$reader = $objReader->load($filename);
		/*$ok = $this->validateUploadRomanian( $reader );
		if (!$ok) {
			return FALSE;
		}*/
		$this->clearCacheRomanian();
		
		$ok = $this->uploadRomanianProducts( $reader, $database );
		if (!$ok) {
			return FALSE;
		}
		chdir( '../../..' );
	...
	function populateRomanianWorksheet( &$worksheet, &$database, $languageId, &$boxFormat, &$textFormat )
	{
		// Set the column widths
		$j = 0;
		$worksheet->setColumn($j,$j++,max(strlen('product_id'),4)+1);
	...
	function populateOptionsWorksheet( &$worksheet, &$database, $languageId, &$priceFormat, &$boxFormat, &$weightFormat, $textFormat )
	{
		...
		if ($hasOptionValueImageField) {
			$worksheet->setColumn($j,$j++,max(strlen('image'),20)+1);
		}
	...
	function downloadRomanian() {
		...
		// sending HTTP headers
		$workbook->send('backup_romanian_products.xls');
		
		// Creating the products worksheet
		$worksheet =& $workbook->addWorksheet('RomanianProducts');
		$worksheet->setInputEncoding ( 'UTF-8' );
		$this->populateRomanianWorksheet( $worksheet, $database, $languageId, $boxFormat, $textFormat );
		$worksheet->freezePanes(array(1, 1, 1, 1));
